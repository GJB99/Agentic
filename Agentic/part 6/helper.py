"""
Description (helper.py):
This script encapsulates the essay writing agent's logic within an ewriter class and provides a Gradio-based graphical user interface for interaction via the writer_gui class.
The ewriter class:
Initializes the OpenAI model, Tavily client, and defines all necessary prompts for planning, writing, research, and reflection.
Constructs and compiles the LangGraph state machine for the essay writing workflow, including nodes for each step (planner, research_plan, generate, reflect, research_critique) and conditional logic for the revision loop.
Configures the graph with an in-memory SQLite checkpointer for state persistence and sets interrupt points after each node, facilitating step-by-step control through the UI.
The writer_gui class:
Takes the compiled graph from an ewriter instance.
Creates a multi-tab Gradio interface:
Agent Control: Allows users to input an essay topic, start a new essay generation process, or continue an existing one. It displays live logs and the current status of the agent (active thread, last/next node, revision number, step count). Users can specify nodes after which the agent should pause.
State Inspection/Modification Tabs (Plan, Draft, Critique, Content, Queries): Enables viewing and modifying the respective parts of the agent's current state. Changes are saved back to the agent's state.
Thread & History Management: Allows switching between different conversation threads (essays) and viewing the history of states for the current thread. Users can "rewind" the current thread to a selected historical state, effectively branching or re-running from that point.
Manages agent execution by invoking the graph, handling interruptions, and updating the UI components with the agent's progress and outputs. It also handles thread management to keep different essay writing sessions separate.
"""
